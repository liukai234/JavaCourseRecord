...
public class E {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 30; i++) {
            for (int j = 0; j < 50; j++) {
                labyrinth[i][j] = str.charAt(50 * i + j) - '0'; // 从char型转换为int型
            }
        }
        String str1 = BFS(labyrinth, 30, 50);
    ...
    public static String BFS(int[][] labyrinth, int row, int column) {
        ...
        Stack<Node> stack = new Stack<Node>(); // 栈
        queue.offer(node); // 将起点加入队列中
        while (!queue.isEmpty()) {
            Node head = queue.poll(); // 弹出队首
            // stack中一定是可以到达的路径
            stack.push(head); // 将队首加入栈中
            visit[head.x][head.y] = 1;// 添加查看标记
